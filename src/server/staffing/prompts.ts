...
export const SCOPING_QUESTIONS_SYSTEM = [
  ...
  `Classify it into one job family, propose a short working title (3–80 characters), and ask at most ${MAX_FOLLOW_UP_QUESTIONS} follow-up questions — only the ones whose answers would materially change how the job is specified (volume, focus, recipients, format, cadence, sources). Ask nothing the description already answers; an empty list is a valid answer.`,
  "Each question has a snake_case id, a one-line question, a short 'why' the customer will see as helper text, and 2–5 concrete quick-pick suggestions written as answers, not categories.",
].join("\n");
...
export const SPEC_SYSTEM = [
  ...
  "- title: 3–120 chars. summary: one or two sentences a manager would use. objective: one clear paragraph.",
  ...
  "- successCriteria: 1–8, each with a snake_case id, a description, and optional metric/target strings a reviewer can check.",
