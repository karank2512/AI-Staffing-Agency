...
export interface CredentialActionsProps {
  ...
  label: string;
  docsUrl: string;
  /** Display names of the tools this key powers, for the dialog copy and the success toast. */
  usedBy: string[];
...
/**
 * "Add / Replace key" dialog plus "Remove" confirmation for one credential row. The key is typed masked, goes
 * to the server once, and never comes back — the row re-renders with the vault's metadata instead.
 * Rendered only for roles that hold `credentials.manage`; the server enforces it regardless.
 */
export function CredentialActions({ name, label, docsUrl, usedBy, stored }: CredentialActionsProps) {
    ...
    <>
        ...
        <DialogContent className="sm:max-w-md" showCloseButton={!pending}>
          <form onSubmit={submit} className="space-y-5">
            <DialogHeader>
              ...
              <DialogDescription>
                Stored for this workspace and only ever read by {tools || "the tools that need it"}. It is never
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-footnote text-muted-foreground">
                  Get one at{" "}
                  <a href={docsUrl} target="_blank" rel="noopener noreferrer nofollow" className="text-link hover:underline">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Team key, rotated Sep 2026"
                  value={customLabel}
      ...
      {stored ? (
        <ConfirmDialog
          ...
          description={
            tools
              ? `${tools} will fall back to the server's key if one is set, otherwise to simulated results. You can add a key again any time.`
